...
count_d = np.squeeze(np.array(m_er.sum(axis=0)))

dfa = pd.DataFrame(count_a, columns = ["Indegree"])
dfb = pd.DataFrame(count_b, columns = ["Indegree"])
dfc = pd.DataFrame(count_c, columns = ["Indegree"])
dfd = pd.DataFrame(count_d, columns = ["Indegree"])

fig, axes = plt.subplots(4,1, sharex = True, figsize = [8.4, 6.4])
sns.histplot(x = "Indegree", data = dfa, ax = axes[0], bins = np.arange(320))
sns.histplot(x = "Indegree", data = dfb, ax = axes[2], bins = np.arange(320))
sns.histplot(x = "Indegree", data = dfc, ax = axes[3], bins = np.arange(320))
sns.histplot(x = "Indegree", data = dfd, ax = axes[1], bins = np.arange(320))
...
# axes[1].annotate("BE count: " + str(bm_er.count_nonzero()), (40,5000))
# axes[2].annotate("BE count: " + str(bm_adjusted.count_nonzero()), (40,3000))
